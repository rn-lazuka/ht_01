...
const blogValidations = [
    ...
    body('websiteUrl').isString().withMessage('Website URL must be a string').matches(/^https:\/\/([a-zA-Z0-9_-]+\.)+[a-zA-Z0-9_-]+(\/[a-zA-Z0-9_-]+)*\/?$/).withMessage('Website URL must be a valid URL with https protocol'),
];
